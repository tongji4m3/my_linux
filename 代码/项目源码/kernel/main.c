...
void TestA()
{
    ...
    while (1)
    {
        printf("[root@tongji4m3 %s]# ", currentDir);//模拟linux的命令行交互
        ...
        if (rdbuf[pointer] != 0)  // 指令后有参数
        {
            pointer++;
            int len = pointer;
            while (rdbuf[pointer] != 0 && rdbuf[pointer] != ' ')  //读取指令第一个参数
            {
                args1[pointer - len] = rdbuf[pointer];
                pointer++;
            }
            args1[pointer - len] = 0;//读取完了第一个参数
        }
        if (rdbuf[pointer] != 0)  // 指令后有参数2
        {
            pointer++;
            int len = pointer;
            while (rdbuf[pointer] != 0 && rdbuf[pointer] != ' ')  //读取指令第二个参数
            {
                args2[pointer - len] = rdbuf[pointer];
                pointer++;
            }
            args2[pointer - len] = 0;
        }
